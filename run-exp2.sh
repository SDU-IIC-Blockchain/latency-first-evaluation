...
for ii in $(seq 0 1 $repeat_count); do
  ...
  echo "exp2-count$ii" | (cd "$ETH_EXP_SCRIPTS_FOLDER" && ./run_geth.sh)
  echo 'sleep 5s...' && sleep 5s

  ACCOUNT_NUM=$account_num python3 ./exp2-init.py
  for i in $(seq 0 2 $(expr $account_num - 1)); do
    MY_ID=$i screen -dm bash -c 'python3 ./exp2-commit.py | tee ./result/exp2/exp2-commit-'$i-count-$ii.txt
  ...
  while true; do
    num=$(ps -ef | grep -i "SCREEN -dm bash -c python3 ./exp2-commit.py" | grep -v 'grep' | wc -l)
  ...
  for i in $(seq 0 2 $(expr $account_num - 1)); do
    MY_ID=$i screen -dm bash -c 'python3 ./exp2-proof.py | tee ./result/exp2/exp2-proof-'$i-count-$ii.txt
    echo -n "Starting ($i/$account_num)             "
    printf "\r"
    sleep 0.01s
  done
  while true; do
    num=$(ps -ef | grep -i "SCREEN -dm bash -c python3 ./exp2-proof.py" | grep -v 'grep' | wc -l)
    echo -n "Numbers remaining: $num                "
    printf "\r"
    if [ $num -eq 0 ]; then
      break
  ...
  cp "$ETH_EXP_SCRIPTS_FOLDER/output/exp2-count$ii.txt" ./result/exp2/
  xz ./result/exp2/"exp2-count$ii.txt"
